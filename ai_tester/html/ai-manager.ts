// Interface for window extensions
interface Window {
  aiManager: AIManager
  pongGame: PongGame
}
...
// Redéfinir l'interface GameState pour correspondre à celle de pong.ts
interface Vector2D {
  x: number
  y: number
}
...
// Interface pour PongGame qui correspond à celle de pong.ts
interface PongGame {
  paddlePosition: number
  aiPaddlePosition: number
  ballPosition: Vector2D
...
let apiIntervalId: number | null = null
let movementTimeoutId: number | null = null
let lastApiCallTime = 0
let currentMovementDirection: "up" | "down" | "none" = "none"
...
function isApiConnected(): boolean {
  return isConnected
}

// Fonction pour se déconnecter de l'API
function disconnectFromApi(): void {
  if (apiIntervalId) {
...
function testApiConnection(endpoint: string): Promise<boolean> {
...
function connectToApi(endpoint: string): void {
    if (!endpoint) {
...
function callApi(): void {
  if (!isConnected || !apiEndpoint) return
...
function handleApiResponse(response: ApiResponse): void {
  ...
  ) // Limiter à 1 seconde maximum
}

// Fonction pour appliquer un mouvement
function applyMovement(direction: "up" | "down"): void {
  if (direction === "up") {
...
function simulateMovement(direction: "up" | "down"): void {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  addLogMessage(
    "Format de données envoyé à l'API: { paddlePosition: number, ballPosition: {x,y}, ballDirection: {x,y} }",
  )
})
